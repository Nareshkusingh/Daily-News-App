...
class NewsApp {
    ...
    buildUrl() {
        // 🔥 GNews correct URLs
        if (this.state.query) {
            return `${this.BASE_URL}/search?q=${this.state.query}&max=10&token=${this.API_KEY}`;
        }

        return `${this.BASE_URL}/top-headlines?country=us&topic=${this.state.category}&max=10&token=${this.API_KEY}`;
    }
    ...
    renderNews(articles) {
        ...
        articles.forEach(article => {
            ...
            // ✅ FIXED IMAGE
            const fallback = `https://picsum.photos/400/300?random=${encodeURIComponent(article.title)}`;

            const img = `
              <img src="${article.image || fallback}" 
                   class="news-image"
